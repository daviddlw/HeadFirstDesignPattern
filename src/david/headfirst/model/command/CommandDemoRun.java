...
public class CommandDemoRun {
	...
	public static void commandFullyDemoRun() {
		...
		List<ICommand> onButtons = Arrays.asList(new ICommand[] {
				new AirConditionOnCommand(new AirCondition("daviddai_on")),
				new KitchenLightOnCommand(new KitchenLight()), new RoomLightOnCommand(new RoomLight()),
				new GarageDoorUpCommand(new GarageDoor()) });
		List<ICommand> offButtons = Arrays.asList(new ICommand[] {
				new AirConditionOffCommand(new AirCondition("daviddai_off")),
